...
# logging.basicConfig(filename="astar_log.txt",
#                     filemode="w",
...
class HamiltonianSearch:
    ...
    def search(self, top_n: int = 3):
        ...
        print(f'Time (pathfinding) {time.time()-st2} s')
        print(f'Total runtime {time.time()-st} s')
        return min_perm, shortest_path # AlgoOutput -> `min_perm`: lowest cost order of visiting all the obstacles starting from starting location; `loc_mn_path`: An array of a path Array of `Node` where each inner path Array is the path from one location to another