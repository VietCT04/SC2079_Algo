...
# logging.basicConfig(filename="search_log.txt",
#                     filemode="w",
...
class AStar:
    ...
    def __init__(
        ...
        self.moves = (
            ( 1,  0, DIST_FW,    Movement.FWD,       fwd,1.5),
            ( 1, -1, DIST_FL45[2], Movement.FWD_LEFT,  fwd_left, 1),
            ( 1,  1, DIST_FR45[2], Movement.FWD_RIGHT, fwd_right, 1),
            (-1,  0, DIST_BW,    Movement.BWD,       bwd, 1.5),
            (-1, -1, DIST_BL45[2], Movement.BWD_LEFT,  bwd_left,1),
            (-1,  1, DIST_BR45[2], Movement.BWD_RIGHT, bwd_right,1),
        )
        self.map = mp
        self.end = None
        self.x_bounds = None
    ...
    def _expand(
        ...
        for v, s, d, mv, f, speed in self.moves:
            nxt_pos = f(st)
            ...
            penalty = 0
            cost = 0
            if v != node.v or s != node.s:  #Penalty for changing motion
                penalty = PENALTY_STOP

            cost += (d/speed)

            nxt_node = Node(nxt_pos.snap(), nxt_pos, node.g + penalty + cost, euclidean(nxt_pos, self.end), node, v, s, d)            
