...
export const convertThetaToDirectionString = (theta: number) => {
  if (theta === 0) return "East";
  else if (theta === Math.PI / 4) return "NorthEast";
  else if (theta === Math.PI / 2) return "North";
  else if (theta === (3 * Math.PI) / 4) return "NorthWest";
  else if (theta === Math.PI) return "West";
  else if (theta === (-3 * Math.PI) / 4) return "SouthWest";
  else if (theta === -Math.PI / 2) return "South";
  else if (theta === -Math.PI / 4) return "SouthEast";
  return RobotDirection.N;
};
